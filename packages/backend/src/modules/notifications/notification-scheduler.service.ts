...
export class NotificationSchedulerService {
  ...
  async processScheduledNotifications(): Promise<void> {
    try {
      ...
      if (notifications.length > 0) {
        this.logger.log(`Processed ${notifications.length} scheduled notifications`);
      }
    } catch (error) {
  ...
  async retryFailedNotifications(): Promise<void> {
    try {
      const failedNotifications = await this.notificationService.getFailedNotifications();
  ...
  async scheduleNotification(
    ...
    this.logger.log(`Scheduled notification ${notification.id} for ${scheduledFor}`);
    return notification.id;
  }

  async scheduleRecurringNotification(job: ScheduledNotificationJob): Promise<void> {
    this.scheduledJobs.set(job.id, job);
    this.logger.log(`Added recurring notification job ${job.id} with pattern ${job.cronExpression}`);
  }

  async cancelScheduledNotification(notificationId: string): Promise<boolean> {
    // Update notification status to cancelled
    const updated = await this.notificationService.getCustomerNotifications(notificationId, 1);
    if (updated.length > 0) {
