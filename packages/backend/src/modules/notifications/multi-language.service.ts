...
export class MultiLanguageService {
  ...
  private readonly templates: Map<string, TranslationTemplate> = new Map([
    ...
    [
      "price_drop",
      {
        ...
        translations: {
          [Language.ENGLISH]:
            "Price Drop Alert! {{productName}} is now {{newPrice}}",
          [Language.PIDGIN]:
            "Price don drop! {{productName}} na {{newPrice}} now",
          [Language.YORUBA]:
            "Owo ti din! {{productName}} yi ti di {{newPrice}}",
          [Language.HAUSA]:
            "Farashin ya ragu! {{productName}} yanzu {{newPrice}}",
          [Language.IGBO]:
            "Ọnụ ahịa adaala! {{productName}} bụ {{newPrice}} ugbu a",
          [Language.FRENCH]:
            "Baisse de prix! {{productName}} coûte maintenant {{newPrice}}",
        },
    ...
    [
      "delivery_update",
      {
        ...
        translations: {
          [Language.ENGLISH]:
            "Delivery Update: Your order {{orderNumber}} is {{status}}",
          [Language.PIDGIN]:
            "Delivery update: Your order {{orderNumber}} {{status}}",
          [Language.YORUBA]:
            "Igbejade soke: Order {{orderNumber}} rẹ ti {{status}}",
          [Language.HAUSA]:
            "Sabuntawar isar da kaya: Odar {{orderNumber}} ta {{status}}",
          [Language.IGBO]:
            "Mmelite nnyefe: Order {{orderNumber}} gị {{status}}",
          [Language.FRENCH]:
            "Mise à jour livraison: Votre commande {{orderNumber}} est {{status}}",
        },
    ...
    [
      "flash_sale",
      {
        ...
        translations: {
          ...
          [Language.PIDGIN]:
            "Flash sale! {{discount}}% discount for some things",
          [Language.YORUBA]:
            "Titaja kiakia! {{discount}}% ẹdinwo lori awọn nkan yiyan",
          [Language.HAUSA]:
            "Sai da sauri! {{discount}}% rangwame akan zaɓaɓɓun kayayyaki",
          [Language.IGBO]:
            "Ire ngwa ngwa! {{discount}}% mbelata na ihe ndị ahọpụtara",
          [Language.FRENCH]:
            "Vente flash! {{discount}}% de réduction sur les articles sélectionnés",
        },
        variables: ["discount"],
      },
    ],
    [
      "low_stock",
      {
        ...
        translations: {
          [Language.ENGLISH]:
            "Hurry! Only {{quantity}} left of {{productName}}",
          [Language.PIDGIN]:
            "Sharp sharp! Only {{quantity}} remain for {{productName}}",
          [Language.YORUBA]:
            "Yara! {{quantity}} nikan ni o ku fun {{productName}}",
          [Language.HAUSA]:
            "Gaggawa! {{quantity}} ne kawai ya rage na {{productName}}",
          [Language.IGBO]:
            "Mee ngwa ngwa! Naanị {{quantity}} fọdụrụ maka {{productName}}",
          [Language.FRENCH]:
            "Dépêchez-vous! Il ne reste que {{quantity}} de {{productName}}",
        },
    ...
    [
      "loyalty_reward",
      {
        ...
        translations: {
          [Language.ENGLISH]:
            "Congratulations! You earned {{points}} loyalty points",
          ...
          [Language.FRENCH]:
            "Félicitations! Vous avez gagné {{points}} points de fidélité",
        },
        variables: ["points"],
      },
    ],
    ...
    [
      [
        "product_available_message",
        {
          ...
          translations: {
            [Language.ENGLISH]:
              "Great news! {{productName}} is now available in our store. Get yours before it runs out!",
            [Language.PIDGIN]:
              "Good news! {{productName}} don dey available for our shop. Buy your own before e finish!",
            [Language.YORUBA]:
              "Iroyin dara! {{productName}} ti wa ninu ile itaja wa bayi. Gba tirẹ ki o to tan!",
            [Language.HAUSA]:
              "Labari mai dadi! {{productName}} yana samuwa a kantin mu yanzu. Ka saya naka kafin ya ƙare!",
            [Language.IGBO]:
              "Ozi ọma! {{productName}} dị na ụlọ ahịa anyị ugbu a. Nweta nke gị tupu ọ gwụ!",
            [Language.FRENCH]:
              "Bonne nouvelle! {{productName}} est maintenant disponible dans notre magasin. Obtenez le vôtre avant qu'il ne soit épuisé!",
          },
          variables: ["productName"],
        },
      ],
      [
        "order_confirmed_message",
        {
          ...
          translations: {
            [Language.ENGLISH]:
              "Thank you for your order! Order {{orderNumber}} has been confirmed and will be processed within 24 hours.",
            [Language.PIDGIN]:
              "Thank you for your order! Order {{orderNumber}} don confirm and we go process am within 24 hours.",
            [Language.YORUBA]:
              "O ṣeun fun order rẹ! Order {{orderNumber}} ti gba ati pe a o ṣe ẹ laarin wakati 24.",
            [Language.HAUSA]:
              "Na gode da odar ku! An tabbatar da odar {{orderNumber}} kuma za a sarrafa ta cikin sa'o'i 24.",
            [Language.IGBO]:
              "Daalụ maka order gị! E kwenyerela order {{orderNumber}} ma a ga-ahazi ya n'ime awa 24.",
            [Language.FRENCH]:
              "Merci pour votre commande! La commande {{orderNumber}} a été confirmée et sera traitée dans les 24 heures.",
          },
          variables: ["orderNumber"],
        },
      ],
  ...
  async translateNotification(
    ...
    if (!titleTemplate) {
      this.logger.warn(
        `Translation template not found for key: ${templateKey}`
      );
      return {
    ...
    let title =
      titleTemplate.translations[language] ||
      titleTemplate.translations[Language.ENGLISH];
    let message = messageTemplate
      ? messageTemplate.translations[language] ||
        messageTemplate.translations[Language.ENGLISH]
      : title;
  ...
  async getTranslationTemplate(
    key: string
  ): Promise<TranslationTemplate | undefined> {
  ...
  formatCurrency(amount: number, language: Language): string {
    const formatters = {
      [Language.ENGLISH]: new Intl.NumberFormat("en-NG", {
        style: "currency",
        currency: "NGN",
      }),
      [Language.PIDGIN]: new Intl.NumberFormat("en-NG", {
        style: "currency",
        currency: "NGN",
      }),
      [Language.YORUBA]: new Intl.NumberFormat("yo-NG", {
        style: "currency",
        currency: "NGN",
      }),
      [Language.HAUSA]: new Intl.NumberFormat("ha-NG", {
        style: "currency",
        currency: "NGN",
      }),
      [Language.IGBO]: new Intl.NumberFormat("ig-NG", {
        style: "currency",
        currency: "NGN",
      }),
      [Language.FRENCH]: new Intl.NumberFormat("fr-NG", {
        style: "currency",
        currency: "NGN",
      }),
    };
    ...
    return (
      formatters[language]?.format(amount) ||
      formatters[Language.ENGLISH].format(amount)
    );
  }
  ...
  formatDate(date: Date, language: Language): string {
    ...
    return (
      formatters[language]?.format(date) ||
      formatters[Language.ENGLISH].format(date)
    );
  }
}
